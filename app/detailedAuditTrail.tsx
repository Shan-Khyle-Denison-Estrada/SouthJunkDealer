...
import { auditTrails, inventory, inventoryTransactionItems, transactionItems, transactions } from '../db/schema';
...
export default function DetailedAuditTrail() {
    ...
    const loadData = async () => {
        try {
            ...
            // 1. Fetch Audit Details + Inventory Batch Info
            ...
            if (auditResult.length > 0) {
                setAuditDetails(auditResult[0]);
                
                // 2. Fetch Line Items (Source History) for this Batch
                const invId = auditResult[0].inventoryId;
    ...
    if (!auditDetails) {
        return (
            <View className="flex-1 justify-center items-center">
                ...
                <TouchableOpacity onPress={() => router.back()} className="mt-4 p-3 bg-blue-600 rounded-md">
            ...
            <View className="flex-1 gap-4 py-4">
                ...
                <View className="flex-row gap-4">
                        ...
                        <View className="flex-row gap-4 h-20">
                            <View className="flex-1">
                                ...
                                <TextInput 
                                    ...
                                    value={auditDetails.prevWeight ? `${auditDetails.prevWeight} kg` : "-"}
                                    editable={false}
                                />
                            </View>
                            <View className="flex-1">
                                ...
                                <TextInput 
                                    className={`flex-1 rounded-md px-3 font-bold text-lg border ${auditDetails.status === 'Adjusted' ? 'bg-yellow-50 border-yellow-400 text-blue-700' : 'bg-gray-200 border-gray-200 text-gray-400'}`}
                                    value={auditDetails.newWeight ? `${auditDetails.newWeight} kg` : "-"}
                                    editable={false}
                                />
                            </View>
                        </View>

                        {/* Row 2: Audit Notes (Scrollable text for long notes) */}
                        <View className="h-40">
                    ...
                    <View className="flex-1">
                        ...
                        <View className="bg-gray-300 flex-1 rounded-md border-2 border-dashed border-gray-400 justify-center items-center overflow-hidden">
                            {auditDetails.evidenceUri ? (
                                <Image 
                                    source={{ uri: auditDetails.evidenceUri }} 
                                ...
                                <View className="items-center">
                                    ...
                                    <Text className="text-gray-500 font-semibold mt-2 text-xs">No Image</Text>
                                </View>
                            )}
                        </View>
                    </View>
                ...
                <View className="flex-1 mt-1">
                    <Text className="text-gray-600 font-bold mb-1 text-xs">Batch Source History</Text>
                    <View className="bg-white flex-1 rounded-md border border-gray-200 overflow-hidden">
                        <View className="flex-row bg-gray-800 p-3">
                            <Text className="flex-1 font-bold text-white text-center text-sm">TX ID</Text>
                            <Text className="flex-1 font-bold text-white text-center text-sm">Type</Text>
                            <Text className="flex-1 font-bold text-white text-center text-sm">Date</Text>
                            <Text className="flex-1 font-bold text-white text-center text-sm">Allocated Wt.</Text>
                        </View>
                        
                        <ScrollView nestedScrollEnabled showsVerticalScrollIndicator>
                            {lineItems.length === 0 ? (
                                <View className="p-4 items-center"><Text className="text-gray-400 italic">No source transactions found.</Text></View>
                            ) : (
                                lineItems.map((item, index) => (
                                    ...
                                    >
                                        <Text className="flex-1 text-gray-800 text-center text-xs font-medium">#{item.txId}</Text>
                                        <Text className="flex-1 text-blue-700 text-center text-xs font-bold">{item.type}</Text>
                                        <Text className="flex-1 text-gray-600 text-center text-xs">{item.date}</Text>
                                        <Text className="flex-1 text-gray-600 text-center text-xs">{item.allocated} kg</Text>
                                    </View>
                                ))
                            )}
                        </ScrollView>
                ...
                <View className="h-16 flex-row gap-4 mb-2">
                    ...
                    <TouchableOpacity 
                        className="bg-blue-600 flex-1 justify-center items-center rounded-md active:bg-blue-700"
                    >
